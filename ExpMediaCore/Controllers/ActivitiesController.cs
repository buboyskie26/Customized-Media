    ...
    public class ActivitiesController : ControllerBase
    {
        ...
        public async Task<ActionResult<SampView>> ActivityNotfication()
        {
            ...
            var comments = await (from p in _context.Comments
                                  ...
                                  where py.ActivityUserId == user.Id
            ...
            var activityRequests = await (from f in _context.Activities
                                          join fr in _context.ActivityAttendees
                                          on f.Id equals fr.ActivityId
                                          where fr.IsAccepted == null
                                          where fr.ActivityCreatedUserId == user.Id
                                          select new AttendeesRequest()
                                          {
                                              ActivityAttendeeId = fr.Id,
                                              RequestJoin = fr.DateRequest,
            ...
            var commentReaction = await (from p in _context.CommentReactions
                                         join pu in _context.Comments on p.CommentId equals pu.Id

                                         where (p.CommentCreatedUserId == user.Id && pu.AuthorId==user.Id)
            ...
            var sharedUserNotif = await (from p in _context.SharingActivities
            ...
            var taggedView = await (from p in _context.TagUsers
                                    where p.UserToTagId == user.Id
